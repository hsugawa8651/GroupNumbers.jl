...
@testset "GroupNumbers.jl" begin
    ...
    @testset "groupby2 emit=Int64" begin
        ...
        @testset "empty arrays" begin
            ...
            @test_throws ErrorException groupby2(Union{}[]; emit = Int64)
        end
    ...
    @testset "groupby2_dict emit=Int64" begin
        ...
        @testset "empty arrays" begin
            ...
            @test_throws ErrorException groupby2_dict(Union{}[]; emit = Int64)
        end
    ...
    @testset "groupby_numbers" begin
        ...
        @testset "empty arrays" begin
            test_groupby_numbers([], Tuple{Any,Vector{Any}}[])
        ...
        @testset "not group" begin
            @test_throws MethodError collect(groupby_numbers("ABC"))
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
            test_groupby_numbers([10, 20, 30], [[10], [20], [30]])
        end

        @testset "group" begin
            @test_throws MethodError collect(groupby_numbers("ABBC"))
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
    ...
    @testset "groupby_numbers emit=Int64" begin
        ...
        @testset "empty arrays" begin
            ...
            @test_throws ErrorException groupby_numbers(Union{}[]; emit = Int64)
        end
        @testset "singletons" begin
            test_groupby_numbers("A", [[65]]; emit = Int64)
            test_groupby_numbers([10], [[10]]; emit = Int64)
        end

        @testset "not group" begin
            @test_throws MethodError collect(groupby_numbers("ABC"; emit = Int64))
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
            test_groupby_numbers([10, 20, 30], [[10], [20], [30]]; emit = Int64)
        end

        @testset "group" begin
            @test_throws MethodError collect(groupby_numbers("ABBC"); emit = Int64)
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
    ...
    @testset "groupby_numbers_dict" begin
        ...
        @testset "not group" begin
            ...
            @test_throws MethodError test_groupby_numbers_dict(
                "ABC",
                [('A', [1]), ('B', [2]), ('C', [3])],
            )
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
    ...
    @testset "groupby_numbers_dict_indices" begin
        ...
        @testset "not group" begin
            ...
            @test_throws MethodError test_groupby_numbers_dict_indices(
                "ABC",
                [('A', [1]), ('B', [2]), ('C', [3])],
            )
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
    ...
    @testset "groupby_numbers_indices" begin
        ...
        @testset "not group" begin
            ...
            @test_throws MethodError test_groupby_numbers_indices("ABC", [[1], [2], [3]])
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
        ...
        @testset "group" begin
            @test_throws MethodError test_groupby_numbers_indices(
                "ABBC",
                [[1], [2, 3], [4]],
            )
            # ERROR: MethodError: no method matching isapprox(::Char, ::Char)
